<header>
  <nav class="navbar navbar-expand-lg navbar-light bg-danger ">
    <div class="container-fluid">
      <img class="navbar-brand text-light" src="style/144.png" width="50px"><span class="nomrestaurant">REstaurant S-P</span>
      <button class="navbar-toggler bg-light" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon bg-light"></span>
      </button>
      <div class="collapse navbar-collapse " id="navbarNav">
        <ul class="navbar-nav ">
          <li class="nav-item">
            <a class="nav-link active text-light accueil" aria-current="page" href="index.php">Accueil</a>
          </li>
          <li class="nav-item">
            <a class="nav-link text-light decouvrir" href="decouvrir.php">Decouvrir</a>
          </li>
          <li class="nav-item">
            <a class="nav-link text-light apropos" href="menu.php">Menu</a>
          </li>
          <li class="nav-item">
            <a class="nav-link text-light commande" href="commande.php">Passer une commande</a>
          </li>
          <li class="nav-item">
            <a class="nav-link text-light apropos" href="contact.php">Contactez-nous</a>
          </li>

          <li class="nav-item">
            <a class="nav-link text-light apropos" href="connexion.php">Connexion</a>
          </li>
        </ul>
      </div>
    </div>
  </nav>
</header>

<script src="js/bootstrap.js">

</script>